...
import os
import sys
sys.path.append(os.path.abspath('external/ZeroShape'))
import shutil
import importlib
import cv2
...
def get_image(opt, image_name, mask_name):
    image_fname = os.path.join(opt.datadir, 'images_cropped', image_name)
...
def prepare_data(opt):
    datadir = opt.datadir
    image_names = [name for name in os.listdir(os.path.join(datadir, 'images_cropped')) 
